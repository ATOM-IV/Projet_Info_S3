...
void creation_fichier_reponse_2(Arbre* A, int appel) {
...
int main(){
    ...
    Arbre* A = NULL;
    int colonne_condition = 3;                              // colonne_condition = 2 / 3 / 4
    long long * capacite = malloc(sizeof(long long));
    long long i = 1;

    while(Ajout_Station_AVL(&A, colonne_condition, i ,capacite,0) != 0){i++;}
    creation_fichier_reponse_3(A, 0);
    //creation_fichier_reponse_4(A,4, 0);
    parcoursInfixe(A);
    return 0;
}